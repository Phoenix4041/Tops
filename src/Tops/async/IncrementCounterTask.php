...
final class IncrementCounterTask extends AsyncTask {
	...
	public function onRun(): void {
		...
		try {
			...
			$db->busyTimeout(5000);
			$stmt = $db->prepare(
				"INSERT INTO player_stats (name, display_name, {$this->column}) VALUES (:name, :displayName, :amount)
				ON CONFLICT(name) DO UPDATE SET {$this->column} = {$this->column} + :amount, display_name = :displayName"
			);
			if ($stmt === false) {
